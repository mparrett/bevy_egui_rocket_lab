...
fn spawn_lab_room(
    ...
    commands.spawn((
        LabRoom, DespawnOnExit(AppState::Lab),
    ...
    // Back wall (z = -half_z) with window cutout
    let win_w = 2.0_f32;
    ...
    commands.spawn((
        ...
        Transform::from_xyz(0.5, ROOM_HEIGHT - 0.3, 0.3),
    ));
...
fn enter_lab(
    mut outdoor_query: Query<&mut Visibility, With<OutdoorMarker>>,
...
fn enter_launch(
    ...
    // Outdoor camera defaults
    *camera_properties = CameraProperties::default();
    if let Ok((transform, _, _, _)) = rocket_query.single() {
        camera_properties.target = transform.translation;
